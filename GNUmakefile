...
website-lint:
	...
	@misspell -error -source=text website/ || (echo; \
		...
		exit 1)
	@docker run --rm -v $(PWD):/markdown --platform=linux/amd64 06kellyjac/markdownlint-cli website/docs/ || (echo; \
...
website-lint-fix:
	...
	@docker run --rm -v $(PWD):/markdown --platform=linux/amd64 06kellyjac/markdownlint-cli --fix website/docs/
