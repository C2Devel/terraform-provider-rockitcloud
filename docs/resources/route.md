...
~> **Note on route tables and routes** Terraform currently provides both a standalone route resource and a [`aws_route_table`](route_table.md) resource with routes defined inline.
At this time you cannot use a route table with inline routes in conjunction with any route resources.
