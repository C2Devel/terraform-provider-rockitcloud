...
* `address` - (Optional, Forces new resource, String) The IP address from an EC2 BYOIP pool.
...
* `tags` - (Optional, Editable, Map of strings) Key-value pairs to assign to the EIP.
...
* `vpc` - (Optional, Editable, Boolean, **Deprecated**) Indicates whether the EIP is for use in a VPC.

~> **Note** The `vpc` argument is deprecated.
Its value is ignored: all EIPs are for use in a VPC.

~> **Note** If both `address` and `public_ipv4_pool` are specified, `address` will be used in the case both options are defined as API only requires one or the other.
...
* `delete` - (Default `3 minutes`) Used when releasing the EIP.
