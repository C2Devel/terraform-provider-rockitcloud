...
Manages an Amazon Machine Image (AMI).
If you just want to share an existing image with another project, it's better to use [`aws_ami_launch_permission`](ami_launch_permission.md) instead.
...
* `virtualization_type` - (Optional, Forces new resource, String) A keyword to choose what virtualization mode the created instances will use.
...
* `volume_size` - (Optional, Forces new resource, Integer) The size of created volumes in GiB.
    * _Constraints:_ Required unless `snapshot_id` is set.
...
`boot_mode`, `deprecation_time`, `ebs_block_device.encrypted`, `ebs_block_device.kms_key_id`, `ebs_block_device.outpost_arn`, `ebs_block_device.throughput`, `ena_support`, `ephemeral_block_device`, `hypervisor`, `image_location`, `kernel_id`, `platform_details`, `ramdisk_id`, `usage_operation`.
